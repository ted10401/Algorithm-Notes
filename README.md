...
## 時間複雜度 O(f(n))
假如執行時間 <img src="https://github.com/ted10401/Algorithm-Notes/blob/main/Images/%E6%99%82%E9%96%93%E8%A4%87%E9%9B%9C%E5%BA%A6%20-%20%E5%9F%B7%E8%A1%8C%E6%99%82%E9%96%93.png" width="400" />
則時間複雜度為 <img src="https://github.com/ted10401/Algorithm-Notes/blob/main/Images/%E6%99%82%E9%96%93%E8%A4%87%E9%9B%9C%E5%BA%A6.png" width="400" />


## 常見演算法
### 分治法 Divide and conquer Algorithm
將大問題分割成多個子問題，降低解決問題的複雜度
如快速排序法(Quick Sort)、遞迴演算法(Recursion)、大整數相乘法

基本思維
1. 分解：將問題分解為小問題
2. 解決：若小問題可解決則解決小問題，若不行則再次分解
3. 合併：將已解決的小問題透過某種規則合併起來
...
### 貪心法 Greed Algorithm
尋找最優解問題的常用方法，但無法保證解為最優解
每個步驟都用貪心原則，選取目前狀態下最好或最優的解
當局部最優解確定後，就不再進行回溯處理，只有在很少的情況下可以得到真正的最優解

基本思維
1. 建立最優解的模型
2. 將問題分解為一系列子問題，定義子問題的最優解
3. 取得子問題的局部最優解，將子問題堆疊出全域最優解

案例：0-1 背包問題

動態規劃法 Dynamic Programming Algorithm
解決多階段決策問題常用的最優化理論
將每個子問題的答案儲存起來供下次求解時直接取用
...
舉例：3、3、1、1、3、2、3 數列中，出現次數超過 50% 的數是 3

## 暴力法
分別計數各數字的出現次數，出現次數最多的數即為主元素

## 排序法
排序後紀錄各數字的出現次數，當某數字出現次數大於前一個數字的出現次數及更新數字
其中由於某個數字必定大於 N/2，所以可以直接找到 N/2 位置的數，此數字即為主元素
...
## 快速排序法
隨機選擇一數字做為劃分點，小於此數字移動到劃分點左邊，大於此數字移動到右邊
如果劃分點右邊的數字較多則往右邊找新的劃分點，反之往左邊找新的劃分點
如果新的劃分點等於原劃分點，則該數字即為主元素
...
4. 執行合法動作
5a. 若執行合法動作後，狀態是結束狀態則結束搜尋
5b. 若執行合法動作後，狀態不是結束狀態則回到步驟 4
...
尋找增廣路徑 Augment Path 是匈牙利演算法的核心
二分圖中的增廣路徑具有以下性質
1. 路徑中邊的條數是奇數
